...
class IBag(ABC, Generic[T]): #Generic specifies that Bag could hold anything, ABC means abstract class -> 
                                #telling the what, not how. No definitions
    """
    Interface for a Bag (MultiSet) data structure.
    """

    @abstractmethod
    def __init__(self, *items: Optional[Iterable[T]]) -> None: #Optional means type could be provided,
                                                                #can be iterable. Star means any # of parameters
        #Typehint (commenting) v Allows info to show when hovering over
        """
        Initializes the Bag. 
        """
        pass
